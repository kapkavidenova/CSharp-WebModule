...
namespace SharedTrip.Services
{
    public class Validator : IValidator
    {
        public ICollection<string> ValidateUser(RegisterUserFormModel model)
        {
            ...
            if (model.Password.Length < UserMinPassword || model.Password.Length > PasswordMaxLength)
            {
        ...
        public ICollection<string> ValidateTrip(AddTripFormModel model)
        {
            ...
            if (model.Seats < SeatsMinNumber || model.Seats > SeatsMaxNumber)
            {
                errors.Add($"Number of seats is not valid. It must be between {SeatsMinNumber} and {SeatsMaxNumber} characters long.");
            }

            if (model.Description.Length > DescriptionMaxLength)
            {
                errors.Add($"Description must not increase {DescriptionMaxLength}.");
            }

            if (model.Description == null || model.StartPoint== null || model.EndPoint == null)
            {
